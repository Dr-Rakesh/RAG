...
def handle_query(user_input: str, session_id: str) -> str:
    ...
    # Strip user input of leading/trailing whitespace and convert to lowercase for easier comparisons
    cleaned_input = user_input.strip().lower()

    # Regular expression to match purely numeric input (ticket number)
    ticket_number_match = re.fullmatch(r"\d+", cleaned_input)

    # Enhanced regex to detect phrases with trigger words like "IR status", "ticket", and a ticket number
    ticket_status_phrase_match = re.search(r"(ir status|ticket).*?(\d+)", cleaned_input)

    # Check if input is purely numerical (a valid ticket number), contains "IR status" with a number, or the word "ticket" with a number
    if ticket_number_match or ticket_status_phrase_match:
        if ticket_number_match:
            # If the input is purely numerical, assume it's a ticket number
            ticket_number = cleaned_input
        else:
            # Extract ticket number from the phrase containing trigger words and a number
            ticket_number = ticket_status_phrase_match.group(2)

        # Now, we have the ticket number to proceed
        records = get_records_by_ir_id(ticket_number)
        if isinstance(records, list) and records:
            ...
            bot_response = f"Sorry, I couldn't find any details for ticket {ticket_number}."
        
        # Clear any pending question and return the ticket status
        memory_store[session_id]['last_question'] = None
        memory_store[session_id]['ticket_number'] = None  # Clear stored ticket number after use
        return bot_response

    # Detect the intent of the input using OpenAI's intent detection model
    intent = detect_intent(user_input)
    ...
    if memory_store[session_id].get('last_question'):
        ...
        if last_question == "Could you please provide the product name and version number?":
            # Store the product name and version number in the session
    ...
    # No pending question, handle normally based on intent
    if 'help_request' in intent.lower():
        # Check if product version is provided
        if 'product_version' not in memory_store[session_id]:
            follow_up_question = "Could you please provide the product name and version number?"
            memory_store[session_id]['last_question'] = follow_up_question
            return follow_up_question
        # Move to problem description
        return handle_product_version(user_input, session_id)

    elif 'ticket_request' in intent.lower():
        # Ask for ticket number if it wasn't provided
        follow_up_question = "Sure! Please provide your ticket number, and I'll check the status for you."
        memory_store[session_id]['last_question'] = follow_up_question
        return follow_up_question

    # If no relevant information found in our data sources
    bot_response = "Sorry, I couldn't find any relevant information in my data sources. You can visit the [IR website](https://support.sw.siemens.com/en-US) for further assistance."
    return bot_response
